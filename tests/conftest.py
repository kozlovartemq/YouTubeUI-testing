...
def web_driver_init(request):
    ...
    if browser_name == "chrome":
        capa = DesiredCapabilities.CHROME       # Переопределение стратегии загрузки сайта
        capa['pageLoadStrategy'] = "none"       # чтобы Ютуб долго не прогружался
...
def pytest_runtest_makereport(item, call):  # Создает скриншот при падении теста во время выполнения
    outcome = yield
